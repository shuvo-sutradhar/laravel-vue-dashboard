<template>
    <div class="container mx-auto max-w-6xl">
        ...
        <div class="flex justify-between items-center w-full mb-4">
          <div>
            <h1 class="page-heading">{{ $t('team_edit') }}</h1>
          </div>
          ...
          <form @submit.prevent="submit" @keydown="form.onKeydown($event)">
              <div class="bg-white dark:bg-gray-900 rounded p-4 md:p-10 shadow">
                <div class="grid grid-cols-6 gap-6">
                  ...
                  <!-- Profile -->
                  <div class="col-span-6 sm:col-span-3">
                    <label for="profile_picture" class="input-label block">{{ $t('profile_pic') }}</label> 
                    ...
                    <div class="bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-300 text-xs mt-5 ml-3 font-bold px-4 py-2 rounded-lg float-left hover:bg-gray-300 hover:text-gray-600 relative overflow-hidden cursor-pointer">
                        <input type="file" name="profile_picture" @change="uploadPhoto" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" :class="{ 'is-invalid': form.errors.has('profile_picture') }"> {{ $t('change') }}
                    </div>
...
export default {
  ...
  methods: {
      ...
      // GET USER INFO
      async getData() {
        const { data } = await this.form.get(window.location.origin+'/api/team/'+this.$route.params.slug);
        this.form.fill(data.data);
        this.form.role = data.data.role[0].slug;
      },
      ...
      // GET PROFILE PHOTO
      getProfilePhoto(){
            let prefix = (this.form.profile_photo.match(/\//) ? '' : window.location.origin+'/assets/images/profile-pic/');
      ...
      // SUBMIT
      async submit() {
           await this.form.patch(window.location.origin+'/api/team/'+this.$route.params.slug).then((response)=>{
