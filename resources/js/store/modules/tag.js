...
export const getters = {
  ...
  allTags: state => state.allTags ? state.allTags.data : ''
}
...
export const mutations = {
  ...
  //get all tag for assign in service
  [types.FETCH_ALL_TAGS] (state, { tags }) {
    state.allTags = tags
  },
...
export const actions = {
  ...
  // Fetch all Team
  async fetchAllTag({ commit }) {
    const { data } = await axios.get(window.location.origin+'/api/tag')
    commit(types.FETCH_ALL_TAGS, { tags: data });
  },
